...
export const analyzeMeal = async (description: string, imageBase64?: string): Promise<MealAnalysis> => {
  ...
  const prompt = `
    ...
    1. Explique como os valores foram calculados, mencionando os alimentos identificados e suas quantidades estimadas em massa (g).
    2. Forneça um resumo curto e direto da refeição (ex: "Frango grelhado com arroz e brócolis").
  `;
        ...
        config: {
          ...
          responseSchema: {
            ...
            properties: {
              ...
              summary: { type: Type.STRING, description: "Resumo curto da refeição" }
            },
            required: ["kcal", "prot", "carb", "fat_total", "fat_sat", "fiber", "explanation", "summary"]
          }
        }
      });

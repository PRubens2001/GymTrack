...
type Tab = 'plan' | 'log' | 'history' | 'stats';
    ...
    <div className="animate-in">
      <header className="mb-8 flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-sm text-zinc-500 dark:text-zinc-400">Planeje e monitore sua evolução nas pistas.</p>
        </div>
        <nav className="nav-tabs flex gap-2 bg-zinc-200 dark:bg-zinc-900 p-1 rounded-lg self-start max-w-full overflow-x-auto no-scrollbar">
          <button 
            ...
            className={`px-3 sm:px-4 py-2 rounded-md text-xs sm:text-sm font-medium transition-all whitespace-nowrap ${activeTab === 'plan' ? 'bg-white dark:bg-zinc-800 dark:text-zinc-100 shadow-sm' : 'hover:bg-zinc-300 dark:hover:bg-zinc-700 dark:text-zinc-400'}`}
          >
            Planejamento
          </button>
          <button 

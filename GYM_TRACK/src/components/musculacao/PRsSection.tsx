...
const PrModal: React.FC<PrModalProps> = ({ pr, onClose, onSave, onDelete }) => {
  ...
  const [weight, setWeight] = useState<string>(pr?.pr.toString() || '');
  const [reps, setReps] = useState<string>(pr?.rep.toString() || '');
      ...
      <div className="bg-white dark:bg-zinc-900 w-full max-w-md rounded-3xl shadow-2xl overflow-hidden animate-in">
        ...
        <form 
          onSubmit={(e) => {
            ...
            onSave({ group, exercise, pr: parse(weight), rep: parseInt(reps) || 0, date });
          }}
